...
class AddRemoveVlanActions(object):
    ...
    def configure_vlan_range(self, start_vlan, end_vlan, action_map=None, error_map=None):
        """Create vlan entity on the device
        ...
        CommandTemplateExecutor(cli_service=self._cli_service,
                                command_template=vlan_command_template.CONFIGURE_VLAN_RANGE,
                                action_map=action_map,
                                error_map=error_map).execute_command(start_vlan=start_vlan, end_vlan=end_vlan)

    def set_vlan_to_interface(self, tag_type, port_name, vlan=None, action_map=None, error_map=None):
        """ Assign vlan to a certain interface

        :param tag_type: port mode. tagged or untagged
        ...
        if vlan:
            ...
            output = CommandTemplateExecutor(cli_service=self._cli_service,
                                             command_template=vlan_command_template.ASSIGN_VLAN_TO_IFACE,
                                             action_map=action_map,
                                             error_map=error_map).execute_command(tag_type=tag_type,
                                                                                  port_name=port_name)
    ...
    def remove_vlan_from_interface(self, tag_type, port_name, vlan, action_map=None, error_map=None):
        """ Assign vlan to a certain interface
        ...
        self._cli_service.send_command(command="")
